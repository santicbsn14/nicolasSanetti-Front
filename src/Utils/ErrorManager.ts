type ErrorResponse = {
  ...
  export function isHttpError(error: unknown): error is { message: string; response?: { status?: number } } {
  return (
    typeof error === 'object' &&
    error !== null &&
    'message' in error &&
  ...
  export const handleError = (error: any) => {
    ...
    if (error.response) {
     ...
     console.log(error.message)
      ...
      console.error(error)
      return `"No se pudo conectar con el servidor. Verifica tu conexión a internet." ${error.message}`;
    } else {
